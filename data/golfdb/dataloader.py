...
class RandomAffine(object):
    def __init__(self, degrees, shear):
        self.ra = transforms.RandomAffine(degrees=degrees, shear=shear)

    def __call__(self, sample):
        images, labels = sample['images'], sample['labels']
        seq_len, C, H, W = images.size()
        images = torch.reshape(images, (seq_len * C, H, W))
        images = self.ra(images)
        images = torch.reshape(images, (seq_len, C, H, W))
        return {'images': images, 'labels': labels}
