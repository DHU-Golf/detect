...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
    ...
    ax_in.get_xaxis().set_visible(True)
    ax_in.get_yaxis().set_visible(True)
#   ax_in.set_axis_off()
...
### 传入的参数是 poses(原始数据 ?) 应该是多个 pose
    ...
    def update_video(i):
        ...
        # Update 2D poses 开始绘制点
        joints_right_2d = keypoints_metadata['keypoints_symmetry'][1]
        colors_2d = np.full(keypoints.shape[1], 'black')
        colors_2d[joints_right_2d] = 'red'
        if not initialized: # 未初始化
